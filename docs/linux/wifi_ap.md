...
mac=$(ifconfig $ifaceEthName | awk  '/ether/{print $2 ;exit}' |sed 's/\://g')
mac=${var}${mac}
...
echo $password | sudo -S nmcli con add type wifi ifname $ifaceWlanName mode ap con-name WIFI_AP ssid $mac
...
echo $password | sudo -S nmcli con modify WIFI_AP ipv4.addr $gateway/24
